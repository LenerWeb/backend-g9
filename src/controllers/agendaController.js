...
export async function crearAgenda(req, res) {
    ...
    try {
        ...
        // aca ingresara si hay algun error ya sea por la validacion o porque el usuario no existe
        res.status(400).json({
...
export async function listarAgenda(req, res) {
    const usuarioId = req.user._id;
    // left outer join utilizando lo que vendria a ahacer una clausula condicional cuando concuerde que solamente muestre las agendas de ese usuario
    const agendas = await Agenda.aggregate([
        ...
        as: "propietario", // en que columna se mostrara este join, nobre que le ponemos
    });
